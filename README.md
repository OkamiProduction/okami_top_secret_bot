...
- ✅ **Деплой на VPS** — systemd-сервис, отдельные пользователи для запуска (`tgbot`) и CI/CD (`github-deploy`).
- ✅ **CI/CD (GitHub Actions)**:
  - `ci.yml` — сборка и тесты при каждом PR в `main`.
  - `deploy.yml` — автоматический деплой на staging-сервер при пуше в `main`.
  - **Метод деплоя:** Прямая передача бинарника по SSH с выполнением `sudo -u tgbot bash -c 'cat > /opt/tgbot/tgbot && chmod +x ...'`. Это гарантирует корректного владельца (`tgbot`) и не требует ослабления прав.
